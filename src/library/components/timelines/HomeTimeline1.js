...
const HomeTimeline = ({ navigation, scrollY, paddingTop, activeTimeline, headerValue }) => {
  ...
  // GET POSTS "FOR YOU"
  const { error, data, refetch, fetchMore, networkStatus } = useQuery(POSTS_FOLLOWING_QUERY, {
    variables: {
      feed: 'following',
      take: 10,
  ...
  const posts = data ? data.postsFollowing.posts : [];
  const hasNextPage = data ? data.postsFollowing.hasNextPage : false;
  ...
  const onRefresh = async () => {
    ...
    await refreshPostsMyGoals(client);
    ...
    <View style={styles.container}>
      ...
      <Animated.FlatList
        ...
        ListFooterComponent={() => {
          ...
          if (activeTimeline === 1 && fetchingMore) {
            return (
          ...
          [
            {
              nativeEvent: {
                contentOffset: {
                  // y: scrollY,
                  y: activeTimeline === 1 ? scrollY : undefined,
                },
              },
            },
          ],
