...
const ProfileBio = ({ isMyProfile, username }) => {
  ...
  const showAbout = isMyProfile || !!user.about || !!user.location;
    ...
    <View style={styles.content}>
      {showAbout && (
        <View style={styles.contentSection}>
          ...
          {!!user.location && <View style={styles.detailsBox}>{renderLocation()}</View>}
        </View>
      )}
      <Showcase username={username} isMyProfile={isMyProfile} />
      {isInvestor && (
        <View style={styles.contentSection}>
          <View style={{ ...styles.contentHeader, paddingBottom: 10 }}>
