...
const TopicFollowButtonInvest = ({ topicID, showX }) => {
  ...
  useEffect(() => {
    if (dataTopics && dataTopics.userLoggedIn && dataTopics.userLoggedIn.topicsInvest) {
      ...
      if (isFollowing) {
        ...
        updateOneUser({
          variables: {
            ...
            data: {
              topicsInvest: { connect: [{ id: topicID }] },
            },
          },
        });
      } else {
        ...
        updateOneUser({
          variables: {
            where: { id: currentUserId }, // userLoggedIn
            data: {
              topicsInvest: { disconnect: [{ id: topicID }] },
            },
          },
        });
      }
...
export default TopicFollowButtonInvest;
