...
const TopicFollowButtonNetwork = ({ topicID, showX }) => {
  ...
  useEffect(() => {
    ...
    if (!isCancelled && !loadingMutation && dataTopics && dataTopics.userLoggedIn && dataTopics.userLoggedIn.topicsInterest) {
      const topicsIDonly = dataTopics.userLoggedIn.topicsInterest.map((topic) => topic.id);
      ...
      if (isFollowing) {
        ...
        updateOneUser({
          variables: {
            ...
            data: {
              topicsInterest: { connect: [{ id: topicID }] },
            },
          },
        });
      } else {
        ...
        updateOneUser({
          variables: {
            where: { id: currentUserId }, // userLoggedIn
            data: {
              topicsInterest: { disconnect: [{ id: topicID }] },
            },
          },
        });
      }
...
export default TopicFollowButtonNetwork;
