...
import { postPicUpload, postVideoUpload, getTopicID, addMainTopics, getNetworkIDsUser, getTopicFromID, isCustomGoalTest } from 'library/utils';
...
const NewPostModal = ({ navigation, route }) => {
  ...
  const { currentUserId, currentUsername, setShowNetworkActivity, setUploadingPost } = useContext(UserContext);
  ...
  const validateInputs = () => {
    if (!content && !video && images.length < 1) {
      ...
      .then(item => {
        ...
        if (item.mime && item.mime.startsWith('video')) {
          if (item.duration < 61000) {
            ...
            Alert.alert('Oh no!', 'Please select a video 60 seconds or less!', [{ text: 'OK', onPress: () => console.log('OK Pressed') }]);
          }
        } else {
          // if images
          setImages([item.path]);
  ...
  const renderImages = () => {
    if (images.length < 1 || !!mentionText) {
      return null;
    }
    if (images.length === 1) {
      return (
              ...
              <View style={{ flex: 1, paddingRight: 20 }}>
                <TouchableOpacity
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                    <Icon name="map-marker-alt" size={15} color={colors.purp} style={{ paddingRight: 8, paddingBottom: 2, opacity: 0.7 }} />
                    <Text
                      numberOfLines={1}
                      // ellipsizeMode="head"
